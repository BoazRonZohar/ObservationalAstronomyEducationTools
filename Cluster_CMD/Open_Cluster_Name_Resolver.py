# -*- coding: utf-8 -*-
"""
Open_Cluster_Name_Resolver.py

Turns the name you would say out loud into the name the catalogue uses.

Cantat-Gaudin's open cluster catalogue (Vizier J/A+A/640/A1) calls things
NGC_6405, not M6, and writes an underscore where you would leave a space. Type
a Messier number, or an NGC/IC/Collinder/Melotte name in any spacing, and this
returns the catalogue form - or says plainly that the cluster is not in it.

Use it before Cluster_CMD.py, so the name you hand that script is one the
catalogue will actually match.
