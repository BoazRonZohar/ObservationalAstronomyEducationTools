# -*- coding: utf-8 -*-
"""
Photometry_Transit_Eclipse_Mono_Plate_Solved.py
======================

One command for a whole LCO night.

Point it at a folder of BANZAI-reduced frames (`*-e91.fits`). It finds every
target in there, measures it against an ensemble of comparison stars, checks
the result against a second, independent set of positions, and writes ONE
summary page plus a light curve you can plot straight from Excel.

The companion of Photometry_Transit_Eclipse_Color_Plate_Solved.py, which does the same job for the
Kinneret one-shot-colour frames. They are kept apart on purpose: the two
instruments need different things, and the Kinneret script works. Everything
downstream of the measurement - the radius scan, the ensemble, the light
curve, the spreadsheet, the star chart, the target.txt override, the summary -
is imported from it rather than copied, so a fix there reaches here too.

What is different about these frames, and why the code is simpler
-----------------------------------------------------------------

  one filter, one image      No Bayer mosaic, no colour planes, no debayer
                             interpolation error. One frame is one measurement.

  the header is complete     GAIN, RDNOISE, SATURATE, PIXSCALE and the seeing
                             (L1FWHM) are all recorded. Nothing is guessed and
                             nothing is asked.

  the plate solution is true CAT-RA/CAT-DEC hold the target's catalogued
                             position and land on the star to 0.2 px. (RA/DEC
                             are where the telescope was pointed, ~18" away -
                             a different keyword, and the wrong one to use.)

  a source catalogue         BANZAI ships its own detections in the CAT
                             extension: position, sky coordinates, flux,
                             FWHM and a quality flag, per frame. Comparison
                             stars are chosen from it instead of running a
                             star finder, and it supplies the second opinion
                             the agreement check needs.

  a bad pixel mask           The BPM extension marks unusable pixels; a star
                             sitting on one is not used.

This file is self-contained. The measurement engine
(Photometry_Transit_Eclipse_Mono_Star_List.py) and the shared measurement and output code
(Photometry_Transit_Eclipse_Color_Plate_Solved.py) are both merged in below, so the script runs on
its own with nothing beside it. Only the packages in requirements.txt are
needed.
...
# -*- coding: utf-8 -*-
"""
Photometry_Transit_Eclipse_Mono_Plate_Solved.py  (FWHM-based aperture version)
=======================================================
...
--------------------------------------------------------------------------------
    python Photometry_Transit_Eclipse_Mono_Plate_Solved.py \
...
# ==========================================================================
# the shared measurement and output code (was Photometry_Transit_Eclipse_Color_Plate_Solved.py)
# ==========================================================================
...
def measure_channel(frames, plane, stars, ref_idx, cfg, route):
    ...
    # The observer's own choice, applied LAST so it wins over every automatic
    # gate. --exclude already dropped stars inside the gate above; --comps
    # names the ensemble outright. See the colour script for why the gates are
    # not enough on their own: they rank on stability over hours and cannot
    # see saturation.
    ...
    if want:
        ...
        if chosen and list(chosen) != list(config["final_comps"]):
            ...
            for c in chosen:
                ...
                if col_ not in df:
                    continue
                sc = _p2p_noise(df[col_].to_numpy(float))
...
def plot_channel(df, config, out_base, title, flip_marks=None, step_marks=None,
    ...
    for c in config["chosen_comps"]:
        ...
        # the filter in the title too: these are opened one after another
        # and without it nothing on the picture says which one it is
...
def read_image(path):
    """The science image, as float.

    One extension, one image - which is the whole reason this file exists as
    a separate script. `fits.getdata` on its own would hand back whichever
    extension it finds first, so the primary is asked for by name."""
    with fits.open(path, memmap=False) as hd:
        return np.asarray(hd[0].data, dtype=float)


def read_catalogue(path):
    """BANZAI's own detections for this frame, or None if it has none."""
    try:
        with fits.open(path, memmap=False) as hd:
            for h in hd[1:]:
                if h.name.upper() == "CAT" and h.data is not None:
                    return np.array(h.data)
    except Exception:
        pass
    return None
...
def read_bpm(path):
    try:
        with fits.open(path, memmap=False) as hd:
            for h in hd[1:]:
                if h.name.upper() == "BPM" and h.data is not None:
...
def scan_folder(folder, pattern="*.fits"):
    """Group every reduced frame under `folder` by the object it points at."""
    files = []
    for root, _dirs, _f in os.walk(folder):
        files += glob.glob(os.path.join(root, pattern))
        files += glob.glob(os.path.join(root, "*.fits.fz"))
    targets, skipped = {}, []
    for p in sorted(set(files)):
        try:
            h = fits.getheader(p)
            if h.get("NAXIS") == 0:                # compressed: the image is in HDU 1
        ...
        if h.get("NAXIS") != 2:
            continue
        if str(h.get("OBSTYPE", "EXPOSE")).upper() not in ("EXPOSE", "SCIENCE"):
            skipped.append((p, f"not a science exposure ({h.get('OBSTYPE')})"))
            continue
...
def classify_frames(paths):
    ...
    A frame is only dropped for a reason that makes it unmeasurable: no plate
    solution, or a plate solution the pipeline itself flagged as failed. Poor
    seeing and thin cloud are not reasons - they are what the error bars are
    for, and throwing away the worst frames of a transit is how a transit gets
    invented."""
    keep, drop = [], []
    for p in paths:
        fr = frame_info(p)
        h = fr["header"]
        if not has_wcs(h):
            drop.append({**fr, "why": "no plate solution in the header"})
            continue
        # WCSERR is BANZAI's own verdict on the solve: 0 is success.
        err = h.get("WCSERR", 0)
        try:
            err = int(err)
        except (TypeError, ValueError):
            err = 0
        if err != 0:
            drop.append({**fr, "why": f"the plate solution failed (WCSERR={err})"})
            continue
        keep.append(fr)
    keep.sort(key=lambda f: (f["mjd"] if np.isfinite(f["mjd"]) else 0.0))
    return keep, drop


def choose_reference(frames, n_sample=12):
    """Which frame the run is measured against: the sharpest of a sample.

    Sharpest, not first. The first frame of a run is routinely the worst one -
    the mount has just settled, the focus may still be running - and every
    star position for the whole night is taken from whichever frame this
    picks, so it is worth picking on evidence."""
    idx = np.unique(np.linspace(0, len(frames) - 1,
                                min(n_sample, len(frames))).astype(int))
    best, best_v = 0, np.inf
    for i in idx:
        h = frames[i]["header"]
        v = float(h.get("L1FWHM", np.nan))
...
def target_coord(header, radec=None):
    ...
    CAT-RA/CAT-DEC, not RA/DEC. RA/DEC is where the telescope was pointed and
    on these frames it sits about 18 arcsec - 24 px - from the star. CAT-RA
    and CAT-DEC hold the catalogued position of the object that was requested,
    and through this WCS they land on it to about 0.2 px."""
    if radec is not None:
        return SkyCoord(radec[0], radec[1], unit=(u.deg, u.deg)), "supplied"
    for ra, dec, why in (("CAT-RA", "CAT-DEC", "CAT-RA/CAT-DEC"),
                         ("OBJCTRA", "OBJCTDEC", "OBJCTRA/OBJCTDEC"),
                         ("RA", "DEC", "RA/DEC (the pointing, not the object)")):
        if ra in header and dec in header and str(header[ra]).strip():
            return SkyCoord(str(header[ra]), str(header[dec]),
                            unit=(u.hourangle, u.deg)), why
...
def pick_stars(header, cat, bpm, img=None, n_comps=12, max_radius=1100, radec=None,
    ...
    nearest detection - and if the observer supplied coordinates, not even
    that: those are used exactly as given. A variable star may be blended,
    ...
    if not np.isfinite(fwhm) or fwhm <= 0:
        fwhm = float(header.get("L1FWHM", 3.0)) / float(header.get("PIXSCALE", 1.0) or 1.0)
    sat = float(header.get("SATURATE", header.get("MAXLIN", np.inf)))

    c, how = target_coord(header, radec)
    tx, ty = [float(v) for v in w.all_world2pix(c.ra.deg, c.dec.deg, 0)]

    # CAT positions are FITS pixels, 1-based; everything here is 0-based.
    cx = np.asarray(cat["x"], float) - 1.0
    cy = np.asarray(cat["y"], float) - 1.0
    cf = np.asarray(cat["flux"], float)
    cp = np.asarray(cat["peak"], float)
    flag = np.asarray(cat["flag"], int) if "flag" in cat.dtype.names else np.zeros(len(cat), int)
    ...
    if locked:
        # Used as given, not snapped onto the nearest light. On a close pair
        # the nearest light is exactly what has to be avoided.
        T = np.array([tx, ty, cf[k], cp[k]])
    elif snap > max(4.0 * fwhm, 15.0):
        raise RuntimeError(
            f"the nearest catalogued source is {snap:.1f} px from "
            f"{how} - the pointing or the plate solution looks wrong")
    else:
        T = np.array([cx[k], cy[k], cf[k], cp[k]])

    # Is anything close enough to the target to worry about?
    others = np.hypot(cx - T[0], cy - T[1])
    others = others[others > 1e-3]
    nn = float(np.min(others)) if len(others) else np.inf
    blend_note = ""
    if nn < 2.0 * fwhm:
        blend_note = (f"another star {nn:.1f} px from the target "
                      f"(FWHM {fwhm:.1f}) - its light is inside the aperture")

    ny, nx = image_shape if image_shape else (int(header["NAXIS2"]), int(header["NAXIS1"]))
    cand = []
    for i in range(len(cat)):
        if np.hypot(cx[i] - T[0], cy[i] - T[1]) < 3:
            continue
        if flag[i] != 0:
            continue                                   # BANZAI flagged it
        if not (40 < cx[i] < nx - 40 and 40 < cy[i] < ny - 40):
            continue
        if cp[i] >= 0.8 * sat:
            continue                                   # near saturation
        if not np.isfinite(cf[i]) or cf[i] <= 0:
            continue
        if cf[i] < 0.2 * T[2] or cf[i] > 12 * T[2]:
            continue                                   # comparable brightness
        sep = np.sort(np.hypot(cx - cx[i], cy - cy[i]))[1]
        if sep < max(5 * fwhm, 25):
            continue                                   # isolated
        if np.hypot(cx[i] - T[0], cy[i] - T[1]) > max_radius:
            continue
        if bpm is not None:
            yy, xx = int(round(cy[i])), int(round(cx[i]))
            r = int(max(3, round(2 * fwhm)))
            if np.any(bpm[max(yy - r, 0):yy + r, max(xx - r, 0):xx + r]):
                continue                               # sitting on bad pixels
        cand.append((cx[i], cy[i], cf[i], cp[i]))
    cand.sort(key=lambda s: -s[2])
    cand = cand[:n_comps]

    names = ["V"] + [f"C{i}" for i in range(1, len(cand) + 1)]
    stars = pd.DataFrame({"name": names,
                          "x": [T[0]] + [s[0] for s in cand],
                          "y": [T[1]] + [s[1] for s in cand],
                          "role": ["target"] + ["comp"] * len(cand)})
    return stars, snap, fwhm, blend_note, how
...
def choose_radii(frames, stars, cfg, n_sample=9, verbose=True):
    """Choose the aperture AND the sky annulus by measurement, not by guess.

    The same test the Kinneret script uses, over the same grid: try a value,
    see how much the comparison stars - which should be constant - disagree
    with each other, and keep whatever makes them quietest. Every combination
    comes out of a single pass over the sample frames, because an aperture sum
    does not depend on the annulus and a sky level does not depend on the
    aperture."""
    ...
    bkg = {k: [] for k in IN}
    for fr in sub:
        img = read_image(fr["path"])
        w = WCS(fr["header"])
        px, py = w.all_world2pix(sky[0], sky[1], 0)
        pos = []
        for gx, gy in zip(np.atleast_1d(px).astype(float),
                          np.atleast_1d(py).astype(float)):
...
def _finish(rows, names, cfg, route):
    """Ensemble and quality gates - the same ones the Kinneret script uses."""
    df = pd.DataFrame(rows)
    if df.empty:
        return df, {}
    target_name = names[0]
    ...
    config = dict(cfg)
    config.setdefault("per_star_fwhm", {n: cfg["median_fwhm"] for n in names})
    config.update(target_name=target_name, chosen_comps=comps, route=route,
                  anchor_name=names[1] if len(names) > 1 else target_name,
                  final_comps=list(comps), comp_stability={}, comp_drift={},
                  comp_tail_offset={}, comp_completeness={},
                  completeness_failed=[], rejected_comps=[],
                  comp_trend_ratio={}, drift_rejected=[])
    ...
    if len(comps) > 2:
        ...
        for c in survivors:
            ...
            if np.isfinite(s) and np.isfinite(d):
                badness[c] = s + abs(d) + (abs(t) if np.isfinite(t) else 0.0)
        # A star is only ever dropped on evidence that it behaved badly; when
        # the test cannot produce a number there is no evidence either way.
        if not badness:
            good, bad = survivors, []
        elif len(survivors) > 2:
            good, bad = select_good_comps(badness, min_gap_ratio=1.4)
        ...
        # Whatever the two filters decided, the operator has the last word.
        good = list(good)          # never mutate the caller's list in place
        for c in list(cfg.get("exclude_comps") or []):
            if c in good and len(good) > 4:
                good.remove(c)
                if c not in drifters:
                    drifters.append(c)
...
def measure(frames, stars, ref_idx, cfg):
    """Measure every frame twice, from one read of each.

    The two routes differ only in where they think each star is:

      wcs   the plate solution of that frame, and nothing else
      cat   the position BANZAI's own detector found in that frame

    They are genuinely independent - one is astrometry, the other is a source
    detector - and if they disagree, something is wrong with the placement
    rather than with the star. Both are measured in the same pass because
    these frames are 52 MB each and reading them twice doubles the slowest
    part of the run for no gain."""
    names = list(stars.name)
    target_name = names[0]
    ref_w = WCS(frames[ref_idx]["header"])
    sky = ref_w.all_pix2world([float(v) for v in stars.x],
                              [float(v) for v in stars.y], 0)
    sky = (np.atleast_1d(sky[0]), np.atleast_1d(sky[1]))

    rows = {"wcs": [], "cat": []}
    for fr in frames:
        img = read_image(fr["path"])
        h = fr["header"]
        cat = read_catalogue(fr["path"])
        w = WCS(h)
        px, py = w.all_world2pix(sky[0], sky[1], 0)
        pred = list(zip(np.atleast_1d(px).astype(float),
                        np.atleast_1d(py).astype(float)))

        base = {"file": os.path.basename(fr["path"]),
                "jd": get_obs_time_jd(h, fr["path"]),
                "airmass": float(h.get("AIRMASS", np.nan)),
                "l1fwhm_px": float(h.get("L1FWHM", np.nan)) /
                             float(h.get("PIXSCALE", 1.0) or 1.0),
                "flipped": False}

        for route in ("wcs", "cat"):
            if route == "cat":
                if cat is None or len(cat) == 0:
                    continue
                cx = np.asarray(cat["x"], float) - 1.0
                ...
                for gx, gy in pred:
                    ...
                    j = int(np.argmin(d))
                    if d[j] <= max(2.5 * cfg["median_fwhm"], 6.0):
                        guesses.append((cx[j], cy[j]))
                    else:
                        guesses.append((gx, gy))       # nothing found; fall back
                        misses += 1
                if misses > len(pred) // 2:
                    continue                            # this frame is not usable here
            else:
                guesses = pred

            row = dict(base)
            row["orientation"] = "normal"
            row["anchor_corr"] = np.nan
            row["anchor_dx"] = float(guesses[1][0] - float(stars.x.iloc[1])) \
                if len(guesses) > 1 else np.nan
            row["anchor_dy"] = float(guesses[1][1] - float(stars.y.iloc[1])) \
                if len(guesses) > 1 else np.nan

            centres, widths = [], []
            for gi, (gx, gy) in enumerate(guesses):
                if gi == 0 and cfg.get("lock_target"):
                    # Held where the supplied coordinates put it, not
                    # centroided: centroiding walks onto the brightest light
                    # in the box, which on a close pair is the wrong star.
                    xr, yr, ok = float(gx), float(gy), True
                else:
                    xr, yr, ok = refine_centroid(img, gx, gy,
                                                     box_half=cfg["centroid_box"])
                centres.append((xr, yr, ok))
                wv = np.nan
                if cfg.get("per_frame_fwhm", True) and ok:
                    try:
                        v = measure_fwhm(img, xr, yr, box_half=cfg.get("fwhm_box", 15))
                        if v is not None and np.isfinite(v) and 1.0 < v < 30:
                            wv = float(v)
                    except Exception:
                        pass
                widths.append(wv)
    ...
    df_w, cfg_w = _finish(rows["wcs"], names, cfg, "wcs")
    df_c, cfg_c = _finish(rows["cat"], names, cfg, "cat")
    return df_w, cfg_w, df_c, cfg_c
...
def main():
    ap = argparse.ArgumentParser(
        description="Measure a whole LCO night with one command.")
    ap.add_argument("--folder", default=None, help="folder with the reduced frames")
    ap.add_argument("--output_dir", default=None)
    ap.add_argument("--n_comps", type=int, default=12)
    ap.add_argument("--exclude", default="",
                    help="comparison stars to drop by hand, e.g. C2,C11. "
    ...
    ap.add_argument("--comps", default=None,
                    help="use exactly these comparison stars, e.g. C3,C7,C9 - "
                         "overrides the automatic choice. The tool also offers "
                         "to ask, whenever a previous run is found.")
    ...
    ap.add_argument("--k_ann_out", type=float, default=None)
    ap.add_argument("--gain", type=float, default=None,
                    help="overrides GAIN from the header")
    ap.add_argument("--ron", type=float, default=None,
                    help="overrides RDNOISE from the header")
    ap.add_argument("--dark", type=float, default=0.0)
    ...
    print("=" * 70)
    print("Photometry_Transit_Eclipse_Mono_Plate_Solved - one command for a whole LCO night")
    ...
    # Choosing again after the evidence, without re-measuring: every flux is
    # already in Details/<filter>_wcs.csv. Offered, never hidden behind a
    # switch - the per-comparison-star plots only exist once a run is over, so
    # the useful moment to choose is exactly here.
    _roots = sorted(glob.glob(os.path.join(args.output_dir, "Output_*")))
    _roots = [r for r in _roots
              if glob.glob(os.path.join(r, "Details", "*_wcs.csv"))]
    if _roots and not args.no_ask_comps:
        if args.reselect:
            _ans = "y"
        else:
            ...
            while True:
                try:
                    _ans = input("  Choose comparison stars from the previous run "
                                 "instead of measuring again? [y/N]: ").strip().lower()
                except EOFError:
                    _ans = ""
                if _ans in ("", "n", "no", "y", "yes"):
                    break
                print(f"     '{_ans}' is not y or n - please answer y or n "
    ...
    if not targets:
        raise SystemExit("No reduced LCO frames with an OBJECT keyword found.")
    ...
    if args.auto_radii is None:
        while True:
            ...
            print(f"    '{ans}' is not y or n - please answer y or n "
                  f"(check the keyboard language)")
    if args.auto_radii:
        print("  -> radii will be measured per target")
    ...
    summary, dropped_all, target_dirs = [], [], {}

    for tname in sorted(targets):
        keep, drop = classify_frames(targets[tname])
        for d in drop:
            dropped_all.append((tname, os.path.basename(d["path"]), d["why"]))
            print(f"  DROP {os.path.basename(d['path'])[-34:]}  -  {d['why']}")
        if len(keep) < 5:
            print(f"  {tname}: too few usable frames, skipping")
            continue

        print(f"\n{tname}  -  {len(keep)} frames")
        tdir = os.path.join(args.output_dir, "Output_" + safe_name(tname))
        os.makedirs(tdir, exist_ok=True)
        target_dirs[tname] = tdir

        over = read_target_file([os.path.dirname(targets[tname][0]), args.folder],
                                  tname, n_targets=len(targets))
        ...
        # read rather than asked for. A command-line value still wins.
        gain = args.gain if args.gain is not None else float(ref_hdr.get("GAIN", 1.0))
        ron = args.ron if args.ron is not None else float(ref_hdr.get("RDNOISE", 0.0))
        ...
        if args.auto_radii:
            try:
                with quiet():
                    k_use, kin_use, kout_use, _v = choose_radii(keep, stars, cfg0)
            except Exception as e:
                print(f"\n    radius scan failed ({e}); using the values given")

        cfg = dict(median_fwhm=fwhm, r_aperture=k_use * fwhm,
                   r_ann_in=kin_use * fwhm, r_ann_out=kout_use * fwhm,
                   k_aperture=k_use, k_ann_in=kin_use, k_ann_out=kout_use,
        ...
        if over:
            ...
            if r_ap is not None or r_in is not None:
                ...
                if 0 < r_ap < r_in < r_out:
                    cfg["fixed_radii_px"] = (float(r_ap), float(r_in), float(r_out))
                    radii_note = (f"target radii supplied "
                                  f"{r_ap:.1f}/{r_in:.1f}/{r_out:.1f} px")

        with quiet():
            df_w, cfg_w, df_c, cfg_c = measure(keep, stars, ref_idx, cfg)
        if df_w.empty:
            print("  nothing measured, skipping")
            continue

        noise = noise_mmag(df_w, cfg_w.get("final_comps", []), None)

        details = os.path.join(tdir, "Details")
        os.makedirs(details, exist_ok=True)
        base = os.path.join(tdir, cname)
        dbase = os.path.join(details, cname)
        with quiet():
            write_outputs(df_w, cfg_w, dbase + "_wcs", ["csv", "xlsx"])
        try:
            write_lightcurve_sheet(df_w, cfg_w, base + "_light_curve.xlsx",
                                     tname, cname, measured_noise=noise)
        except Exception as e:
            print(f"\n    could not write the light-curve sheet: {e}")
        plot_channel(df_w, cfg_w, base + "_wcs",
                       f"{tname}  -  filter {cname}  (positions from the plate solution)",
                       measured_noise=noise, comp_base=dbase + "_wcs")
        if not df_c.empty:
            with quiet():
                write_outputs(df_c, cfg_c, dbase + "_cat", ["csv"])
        try:
            finder_chart(ref_img, stars, cfg_w, os.path.join(tdir, "stars_used.png"),
                           tname, cname, note=blend_note)
        except Exception as e:
            print(f"\n    could not draw the star chart: {e}")

        cmp_ = compare_routes(df_w, df_c, list(stars.name), limits)
        sc = scatter_mmag(df_w)
        rng = target_depth_mag(df_w)
        cr, r_t, r_c, n_c = change_ratio(df_w, cfg_w.get("final_comps", []))
        note = note_src
        for extra in (over_note, radii_note):
            if extra:
                note = (note + "; " if note else "") + extra
        summary.append(dict(target=tname, channel=cname, scatter=sc, noise=noise,
                            rng=rng, snr=cr, n_used=cmp_.get("n_frames_wcs", 0),
                            n_frames=len(keep),
    ...
    write_summary(summary, args.output_dir, dropped_all)
    print("")
    for tname, tdir in sorted(target_dirs.items()):
        try:
            _lines, _s, _c, _t, _tb = comp_star_table(tdir)
            if _lines:
                print("")
        ...
        _tbl = write_comp_star_table(tdir, tname)
        print("")
        print("  " + str(tname) + "  ->  " + os.path.abspath(tdir))
        if _tbl:
            print(f"     comparison-star table: {os.path.basename(_tbl)}")
