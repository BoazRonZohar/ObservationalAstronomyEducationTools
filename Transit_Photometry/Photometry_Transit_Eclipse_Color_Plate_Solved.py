    ...
    for tname in sorted(targets):
        ...
        if args.pair_cap and not args.star_list:
            try:
                ...
                pair = detect_close_pair(
                    ...
                    pair_fwhm)
                # distances are fixed on the sky, so the list is found once
                # and every frame sizes its own ring from it
        ...
        for plane, cname in CHANNELS:
            ...
            pair_cfg = dict(pair_sep_px=pair_sep, pair_ratio=pair_ratio,
                            ...
                            pair_fwhm=pair_fwhm,
                            target_neighbours=target_neigh)
            if target_neigh and args.ring_fix:
                _ri, _ro, _md = choose_annulus(
                    target_neigh, pair_fwhm or fwhm,
                    args.k_ann_in, args.k_ann_out)
                if _md != "unchanged":
                    note_src = ((note_src + "; ") if note_src else "") + (
                        f"a star sits in the sky ring - ring {_md} to "
                        f"{_ri:.1f}-{_ro:.1f} px at the median seeing")
            if not args.ring_fix:
                pair_cfg["target_neighbours"] = []
